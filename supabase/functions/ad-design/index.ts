...
Deno.serve(async (req: Request) => {
  ...
  let b: {
    ...
    count?: number;
    /** المزاج اللونيّ الذي اختاره التاجر (فهرس). غيابه يترك الاختيار
     *  للنموذج. */
    mood?: number;
  };
  ...
  try {
    ...
    const brief = [
      ...
      // المزاج المختار يُبلَّغ للنموذج ولا يُترك له.
      //
      // والعميل يفرضه على ما يعود أصلًا، فهذا السطر ليس حارسًا بل
      // **اتّساقًا**: النموذج يختار الخلفية ويكتب شرحه بناءً على ما
      // يظنّه اللوحة، فإن ظنّها كحليّةً وهي ترابيّة خرج شرحٌ يصف
      // تصميمًا غير الذي سيُرسم، وخلفيةٌ اختيرت لألوانٍ لن تكون.
      typeof b.mood === "number"
        ? `المزاج اللونيّ مفروض: ${b.mood}. الزمه ولا تقترح غيره.`
        : null,
    ].filter(Boolean).join("\n");
